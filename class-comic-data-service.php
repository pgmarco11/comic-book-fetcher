...
class ComicDataService {
    ...
    */
    ...
    public function get_series_issues( $title_id, $current_page = 1, $search = '' ) {
        ...
        // ── Genre fallback from issue detail ──────────────────────────────
        // SeriesRead normally includes genres, but some Metron series may
        // have an empty genres array. Individual IssueRead responses contain
        // an IssueSeries object which also exposes the series genres.
        //
        // Use one of the issues we already fetched so this does not require
        // another issue-list request.
        if (
            is_array( $series ) &&
            empty( $series['genres'] ) &&
            ! empty( $all ) &&
            ...
            $genre_fallback_key = "metron:series_genres:{$title_id}";
            $fallback_genres    = get_transient( $genre_fallback_key );

            if ( $fallback_genres === false ) {

                $issue_id = (int) $all[0]['id'];

                $issue_detail = $this->client->api_get(
                    $this->client->api_base .
                    "issue/{$issue_id}/"
                );

                if (
                    is_array( $issue_detail ) &&
                    empty( $issue_detail['error'] ) &&
                    ! empty( $issue_detail['series']['genres'] ) &&
                    is_array( $issue_detail['series']['genres'] )
                ) {
                    $fallback_genres = $issue_detail['series']['genres'];
                    ...
                    /*
                    * Metron successfully answered, but this issue's series
                    * also has no genres. Cache the negative result briefly.
                    */
                    $fallback_genres = [];
            ...
            if ( ! empty( $fallback_genres ) ) {

                $series['genres'] = $fallback_genres;

                /*
                * Repair the normal cached SeriesRead response too so
                * subsequent page loads don't need the fallback request.
                */
                set_transient(
                    $series_key,
                    $series,
